...
test("getCategoryUrl", () => {
    const f = testManagementSet.getCategoryUrl(opt.host, MngCategoryEnum.RULE, [], { path: "/Client/Important" });
    const expected = "https://api.decisionrules.io/api/rule/?path=/Client/Important";
    expect(f.href).toBe(expected);
});
...
test("getCategoryUrl", () => {
    const f = testManagementSet.getCategoryUrl(opt.host, MngCategoryEnum.RULE, ["123", "new-version"]);
    const expected = "https://api.decisionrules.io/api/rule/123/new-version";
    expect(f.href).toBe(expected);
});

test("getCategoryUrl", () => {
    const f = testManagementSet.getCategoryUrl(opt.host, MngCategoryEnum.RULE, ["lock", "123", "1"]);
    const expected = "https://api.decisionrules.io/api/rule/lock/123/1";
    expect(f.href).toBe(expected);
});
...
test("getCategoryUrl", () => {
    const f = testManagementSet.getCategoryUrl(opt.host, MngCategoryEnum.FOLDER, ["rename", "123"]);
    const expected = "https://api.decisionrules.io/api/folder/rename/123";
    expect(f.href).toBe(expected);
});

test("getCategoryUrl", () => {
    const f = testManagementSet.getCategoryUrl(opt.host, MngCategoryEnum.FOLDER, ["move"]);
    const expected = "https://api.decisionrules.io/api/folder/move";
    expect(f.href).toBe(expected);
});

test("getCategoryUrl", () => {
    const f = testManagementSet.getCategoryUrl(opt.host, MngCategoryEnum.FOLDER, ["find"]);
    const expected = "https://api.decisionrules.io/api/folder/find";
    expect(f.href).toBe(expected);
});
